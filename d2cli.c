		...
		for (int ie = 0; ie < EX; ie++)
			for (int ii = 0; ii < IX; ii++) {
				...
				if (*p != ',' || strcmp(field, mod) != 0) {
					return csvError(mod, class, iq, ie, ii,
							"Model");
				}
				p = scanField(p + 1);
...
_Bool readCSV(struct stat * buf)
{
	...
	for (int c = 0; c < D2CLASSES; c++) {
		if (!readCSVClass(fcsv, modelAllClass[c], "All", classAbbr[c])
		    || !readCSVClass(fcsv, modelUnkClass[c], "Unk",
				     classAbbr[c])) {
			sprintf(line, msgCSVData, fnCSV);
			return false;
		}
...
void readConfig()
{
	...
	if (classSpec && !classPossible)
		for (nClassCompute = 0; nClassCompute < nClassColumns;
		     nClassCompute++)
			classCompute[nClassCompute] =
			    classColumn[nClassCompute];
}
