...
void se2000(double mjd, double *psun_earth, double *psoe, double *pcoe)
{
    ...
    // distance in AU
    double r = 1.00014 - .01671*cos(g*M_PI/180.) - .00014*cos(g2*M_PI/180.);
...
{
    double d2 =
        tk->observer1_object0_mag * sin(an) / sin(M_PI - an - tk->tz);
...
void setupDistanceDependentVectors(tracklet * tk, double d)
{
    ...
    for (int i = 0; i < 3; i++)
        tk->sun_object0[i] =
            tk->sun_observer[0][i] + tk->observer_object0[i];
    tk->sun_object0_magsq = dot3(tk->sun_object0, tk->sun_object0);
    tk->sun_object0_mag = sqrt(tk->sun_object0_magsq);

    // observer1_object0
    for (int i = 0; i < 3; i++)
        tk->observer1_object0[i] =
            tk->sun_object0[i] - tk->sun_observer[1][i];
...
_Bool solveAngleRange(tracklet * tk, double *ang1, double *ang2)
{
    ...
    double cc =
        tk->observer1_object0_magsq * aa - 2. * U / tk->sun_object0_mag;
                    ...
                    if (tk->dTag[iq][ie][ii][ih]) {
                        ...
                        for (int c = 0; c < nClassCompute; c++, cl++) {
                            if (cl->dInClass[iq][ie][ii][ih]
                                ...
                                cl->sumAllInClass
                                    +=
                                    modelAllClass[classCompute[c]][iq][ie]
                                    [ii][ih];
                                cl->sumUnkInClass +=
                                    modelUnkClass[classCompute[c]][iq][ie]
                                    [ii][ih];
                            }
                            if (cl->dOutOfClass[iq][ie][ii][ih]
                                ...
                                cl->sumAllOutOfClass
                                    += (modelAllSS[iq][ie][ii][ih]
                                        -
                                        modelAllClass[classCompute[c]][iq]
                                        [ie][ii][ih]);
                                cl->sumUnkOutOfClass +=
 ...
 */
void
gcFit(gcfparam * gcf, double mjd[gcf->nObs], double sphr[gcf->nObs][2])
{
    int nObs = gcf->nObs;
       ...
       observation * olist, observation * result)
{
    ...
    if (o1 == o2 - 1) {
        ...
        result->mjd = arcsUseAllObs ? (olist[o1].mjd + olist[o2].mjd) * .5
            : pt;
...
void twoObs(tracklet * tk, double rms[])
{
    ...
    if (spaceBased) {
        ...
        memcpy(&tk->obsPair[1], olist + nObs - 1 - is,
               sizeof(observation));
        return;
    }
    ...
    rms[1] =
        oneObs(o2, nObs - 1, o2 == o1 + 1, t83, olist, &tk->obsPair[1]);
}
