...
void *scoreStaged(void *id)
{
	while (1) {
		...
		int len =
		    snprintf(outputLine, outputLineSize, "%s", tk->desig + 5);
		if (rms) {
			// we expect 6 new bytes.  more than that means field overflow
			if (snprintf
			    (outputLine + len, outputLineSize - len, " %5.2f",
			     tk->rms)
			    != 6)
		...
		if (classPossible) {
			...
			for (c = 0; c < D2CLASSES; c++) {
				int cc;
				for (cc = 0;
				     cc < nClassColumns && classColumn[cc] != c;
				     cc++) ;
				if (cc < nClassColumns)
			...
			for (c = 0, cl = tk->class; c < nClassCompute;
			     c++, cl++) {
				if (raw)
					len +=
					    snprintf(outputLine + len,
...
int main(int argc, char **argv)
{
	...
	for (int th = 0; th < cores; th++) {
		...
		ring[th] = tk;
		tk->class =
		    (perClass *) calloc(nClassCompute, sizeof(perClass));
		if (!tk->class)
			fatal(msgMemory);
	}
	...
	while (fgets(line, LINE_SIZE, fobs)) {
		...
		if (line[14] == 's') {
			if (tk->lines
			    && parseMpcSat(line, tk->olist + tk->lines - 1))
				continue;
			pGood = 0;
		} else
